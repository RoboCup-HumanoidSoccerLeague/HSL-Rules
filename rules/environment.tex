...
A Team's Half is the half of the field that is closest to a team's own goal, where they primarily defend against opposing attacks.
...
\begin{figure}[h]
    \centering
    \centerline{\includegraphics[width=0.7\columnwidth]{figs/Figure-1-line-names.png}}
...
\subsubsection{The corner arc}
\label{sec:corner_arc}

A quarter circle from each corner is drawn inside the field of play.
The radius of the arc is 0.5m for the Medium Field and 1.0m for the Large Field. The Small field does not have corner arcs.
...
There are three field sizes that can be used for the proposed divisions, described in Table \ref{tab:field_dim_rule}.

\begin{table}[h]
    \centering
    \begin{tabular}{|c|c|c|}
    \hline
        Field & Width (meters) & Lenght (meters)\\
        \hline
        \hline
...
Table \ref{tab:field_dim} shows the dimensions of the Small, Medium and Large soccer fields and Figures \ref{fig:field_small}, \ref{fig:field_medium} and \ref{fig:field_large} shows the diagram of the three soccer fields. Note that the Middle division can be played on the S-Field or the M-Field, while the Large division can be played on both the M-Field and the L-Field.
More detailed technical drawings are provided in Section \ref{sec:technical_drawings}.
...
\textbf{NOTE THAT WE STILL NEED TO DEFINE THE GOAL DIMENSIONS}
...
\begin{center}
    \begin{table}[h]
        ...
        \begin{tabular}{|l|l|c|c|c|}
            ...
            & (all values in meters) & (former KidSize) & (former AdultSize)& (similar to MSL)\\
            \hline \hline
            A & Field length        & 9.0  & 14.0 & 22.0 \\ 
            \hline
            B & Field width         & 6.0 & 9.0 & 14.0 \\ 
            \hline
            C & Goal length (depth) & 0.6  & 0.6  & 0.6  \\ 
            \hline
            D & Goal width          & 2.6  & 2.6  & 2.4  \\
            \hline
            ...
            \hline
            K & Border strip width (min.) & \multicolumn{3}{c|}{ 1.0 } \\ 
            \hline
            L & Corner Arc radius & none & 0.5 m & 1.0   \\ 
            \hline
            & Line width & \multicolumn{2}{c|}{0.05 }& 0.12\\
            \hline
             & Penalty and center mark size & \multicolumn{2}{c|}{0.10 } & 0.15 \\
            \hline
        \end{tabular}
...
%\hline
%\hline
%Goal width & 1.8 m / 6 FT & 2.4 m / 8 FT & 3 m / 10 FT\\
%\hline
%Goal height & 1.2 m / 4 FT & 1.5-1.6 m / 5 FT& 2 m / 6.5 FT\\
%\hline
%Goal depth & 0.5 to 1 m & 0.7 to 1.2 m & 1 to 2 m\\
%\hline
%\end{tabular}
%\caption{}
%\end{table}
...
% Reinaldo commento - Even with repetition, I think this makes the rule clear, so I removed the cooment below:
...
An object (such as a robot or the ball) is considered \textit{inside} a region of the field if any part of it overlaps or touches the boundary lines that define that region, or if it is fully contained within the region, in the air or on the ground. It is considered \textit{outside} the region only when no part of it, or its downwards projection, remains within or on the boundary lines of that region. This definition applies to any designated area of the field, except the goal area (See Figure~\ref{fig:field_dim}).
...
% removed by Reinaldo as this is repeated from the first part. The definition of field half and center circle was copied there.
...
The balls  for each division are defined in table \ref{tab:balls}.
%
\begin{table}[h]
    \centering
    \begin{tabular}{|c|c|c|}
    \hline
        ...
        \hline
        \hline
        Small & FIFA Mini Ball \footnote{The TCs will decide on only one ball that will be used by all teams during the 2026 competition.} \\
        \hline
