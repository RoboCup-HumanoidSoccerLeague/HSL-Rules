...
During a brief stop, robots remain in place and must not be moved, except for untangling or enforcing the rules. If an exceptional situation requires the robots to be removed from the field, a different call must be made, such as a referee timeout.
...
\begin{enumerate}
    ...
    \item In the event of any offense that directly affects the kick-off (e.g. unduly interfering with the ball's trajectory or with the kicker) during the kick-off procedure, the kick-off is retaken.
\end{enumerate}
...
\begin{enumerate}
    \item The regulations defined in the section \Cref{sec:fouls_misconduct} apply (this includes in particular \Cref{sec:motion_in_set} and \Cref{sec:illegal_positioning}).
    \item In the event of any offense that directly affects the dropped ball during the procedure, the dropped ball is retaken.
\end{enumerate}
...
Teams are ultimately responsible for their robots to play in a safe and sportsmanlike manner. Repeated, \textit{willing} dangerous behavior (including neglecting to correct a safety issue discovered previously), especially over multiple games, may be considered unsporting conduct by the team (see below).
...
If, when a free kick is taken, an opponent is violating the rules above in a way that directly interferes with the free kick (e.g. unduly changing the trajectory of the ball or physically disturbing the kicker), the kick is retaken (in addition to the normal removal penalty).

Note that a defensive robot that had found itself already inside the avoidance zone as the free kick was called cannot be penalized if the kick is taken before it had time to leave, as long as it had been making an effort to move away from the ball. This is an exception to the rule above.
...
Before the penalty shoot-out begins, each team must present to referees up to 6 prepared robots that may participate in the penalty shoot-out. No robots may be added once the penalty shoot-out starts. Robots that will not participate in the shoot-out must not be on the wireless network and must stay outside of the field. All participating robots must be wearing the correct jersey for their player number and no duplicate numbers are permitted.

During each penalty kick of the shoot-out, the robots are controlled by the GameController and the referee's whistle. Robot handlers cannot press buttons on their robots while the penalty kick is in progress. Before the penalty kick begins and after it ends, robot handlers are allowed to press buttons on their robots to switch them between a mode suitable for handling and one suitable for playing, if so required by their safety measures.

\paragraph{Penalty Kick Procedure}
\label{sec:pso_kick}

A penalty kick is carried out with one striker robot and one opposing goalkeeper. Before each penalty kick, both teams must select the robot to participate (as goalkeeper or striker). The team leader communicates the selection to the head referee by privately handing the referee a card with their chosen number. Alternatively, the team leader may communicate their selection verbally if they prefer. After both teams have selected their player, the GameController operator selects the requested striker and goalkeeper robots and the GameController communicates that all non-selected robots are substitutes and should remain inactive. The GameController indicates which team has the striker robot in the current penalty kick.

The referees place the ball on the penalty mark closest to the goal being defended.

Robot handlers of the respective teams place the striker and goalkeeper robots. The striker robot is positioned on the edge of the penalty area, facing the ball and the goal. The goalkeeper is placed with its feet on the goal line and in the center of the goal. Neither robot is permitted to locomote during the \texttt{set} state.
...
In the event of any other infringement that directly interferes with the free kick, the kick is retaken.
...
In the event of any other infringement that directly interferes with the kick, the kick is retaken.
