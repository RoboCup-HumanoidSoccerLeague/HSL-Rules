...
\end{itemize}
The GameController and head referee will indicate this by the call ``Ball Free''. The ball remains in play at all other times.
...
\item play has been stopped by the referee, or game state
\end{itemize}
...
A goal, including own goal, is achieved when the entire ball (not only the center of the ball) goes over the goal-side edge of the goal line, \ie the ball is completely inside the goal (see Section~\ref{sec:inside_outside}).\footnote{
