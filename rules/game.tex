...
Robots that do not follow these rules are penalized for illegal position. Then, when the referee gives the signal to start play:
\begin{itemize}
    ...
    \item the ball is in play when it is kicked and clearly moves, or \qty{\KickOffBallFreeTime}{\second} after the referee has given the signal, whichever happens first. In either case, the GameController and head referee will indicate this by the call ``Ball Free'',
