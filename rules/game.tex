...
\todo[inline]{The goal-point is awarded to the opponent team of the goal that was scored into. }
\todo[inline]{Own goals can be scored even if there was an infringement? }
...
The referee has the authority to take disciplinary sanctions from the moment they enter the field of play until they leave the field of play after the final whistle (including penalty shootouts). Removal penalties, warnings, yellow and red cards are used to discipline players according to the nature of the offense committed.
...
\subsection{Removal Penalty}\label{sec:removal_penalty}
...
\subsection{Definition of Incapable Robots}\label{sec:definition_of_incapable_robots}
...
\subsection{Request for Pick-up}\label{sec:request_for_pick_up}
...
\subsection{Illegal positioning}\label{sec:illegal_positioning}
...
\todo{Add approaching free kicks too?}

\subsection{Ball Holding}\label{sec:ball_holding}
...
\subsection{Untangling}\label{sec:untangling}
...
\subsection{Cautionable offenses}\label{sec:cautionable_offenses}
...
\subsection{Definition of Pushing}\label{sec:definition_of_pushing}
...
\subsection{Sending-off offenses}\label{sec:sending_off_offenses}
...
\subsection{Substituting a Cautioned Player}\label{sec:substituting_a_cautioned_player}
...
\subsection{Penalties against Teams and Humans}\label{sec:penalties_against_teams_and_humans}
...
\begin{itemize}
	\item Offensive, violent or abusive behaviour
...
\begin{itemize}
  ...
  \item When a pushing foul or other infringement is awarded (\cf~\cref{sec:definition_of_pushing}), termed a \emph{Pushing Free Kick}.
\end{itemize}
...
For a \emph{Pushing Free Kick}, the ball is left in place, repositioned only if required by the pushing rules (\cf~\cref{sec:definition_of_pushing}).
...
During a free kick, only the attacking team may approach within the avoidance region of the ball. For a goal kick, the avoidance region is the entire penalty area. For all other free kicks, the avoidance region is division dependent: \FreeKickRadius. \unsure{Should be the whole penalty area for goal kicks?}
...
\subsubsection{Procedural Requirements}

All opponents must stand no less than \FreeKickRadius{} from the point at which the kick-in is taken.
