...
\sublaw{Equipment of human on the field}
...
The referee jersey provided by the event is recommended for all referees but is not mandatory.
...
The head referee should avoid handling the ball (except for placing a ball for kick-off), and avoid handling the robots. Their duty is to monitor and adjudicate the game. The head referee should only handle robots and the ball if absolutely necessary to expedite game-play, where the robot handlers and assistant referees are otherwise occupied or too far away, and only if it doesn't compromise their own safety.
...
The assistant referees assist the head referee in officiating the match, especially with offenses and other events when they have a clearer view. For example, they indicate when the ball has completely crossed over the line of the field, and which team is awarded the resulting goal or free kick. They can also point out rule violations that occurred outside of the head referee's view.
...
The GameController operator announces events that occur automatically in the GameController due to elapsed time, such as the ball coming into play after a kick-off, penalty kick or free kick, or the state changing from \texttt{ready} to \texttt{set}. They also announce when a robot is returning to play from a penalty. Before they actually allow the robot to return, they make sure it is safe for any nearby humans to do so.

Finally, they are also responsible for keeping the time of each half. They should count aloud the remaining seconds in a half once the time remaining is \qty{5}{\second} or less.
...
\subparagraph{Use of the Remote Emergency Button}
Robot handlers are allowed to carry and operate a remote emergency stop button during a game (\cf~\Cref{sec:robot_safety}).
The remote emergency button must be clearly marked.
The remote emergency button must be either: (1) worn around the neck or on the belt of the robot handler, or (2) placed within a clearly marked and designated location of the team technical area. 
Robot handlers must keep their hands clearly away from the button unless the button is actively being pressed. 
%
Robot handlers can only use the remote emergency button in the following two cases: 
(1) command by the main referee for the robot to be stopped and removed from the game;
(2) emergency situations where the robot's behavior poses danger to the humans outside of the field.
%
Robot handlers must not use the remote emergency button to intentionally incapacitate their robots with the aim of gaining advantage in the game.

\sublaw{GameController contact}
...
Each team has one GameController contact, who cannot also be a robot handler. The GC contact role is usually covered by the captain, unless the captain is also a handler, in which case a different team member must be designated.
...
In addition, only the team captains and at most two more people per team are in the Technical Area (\cf~\Cref{sec:technical_area}).
This allows the referees easier communication with the team and the GameController operator gets less disturbed.
...
\paragraph{Assigning team colors.} The teams agree on the jersey colors they will use during the match, following the constraints outlined in \cref{sec:team_markers}. The head referee should respect the teams' wishes as long as they fit within the constraints. If a suitable agreement cannot be reached, the head referee must make the decision if possible, or propose an alternative solution.

\paragraph{Choosing side and kick-off.} The head referee flips a coin. The team who won the flip can decide whether they will take the kick-off or choose which side to defend during the first half. The team who lost the flip gets the other benefit.

\sublaw{Referee -- Team Communication}
\label{sec:referee_team_communication}

During the match, only the team captains are allowed to communicate with the head referee. Requests for pick-up (Section \ref{sec:request_for_pick_up}) are an exception: they may be made only by the team captain or a robot handler.
