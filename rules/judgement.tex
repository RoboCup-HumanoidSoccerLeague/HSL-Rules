...
The handlers are the only people allowed to touch robots of their own teams (except for those that are picked up).

The division rules specify whether robot handlers are allowed, and if so, how many. In divisions that do not allow handlers, the assistant referees take their place, handling both teams' robots.
...
15 minutes before the start of the game, the people who are going to serve as referees meet up to discuss the upcoming game. At least, they must assign among themselves the roles of head referee and GameController operator, with the remaining people acting as assistant referees.
...
\begin{itemize}
    ...
    \item Determining which team is to be given a free kick when the ball goes out of the field (Section ???) and communicating this to the head referee
