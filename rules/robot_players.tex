...
Both the division and the configuration of a team are defined before the competition \improvement{precise modality TBD} and cannot change during the competition itself.
...
In addition to the emergency stop button, robots may only have up to two additional physical or virtual buttons
\info{to be discussed: 4.7.1 allows buttons, community also requests to allow more buttons (NAO). Usage during the game can be restricted}
: One to start the robot behaviour and one to stop the behaviour. 
...
In the Large Division, robot handlers are allowed to carry an additional remote emergency stop button.
...
\improvement{to be discussed, add further safety measures?}
...
The visual sensors, e.g., cameras of the robot shall be arranged such that the combined visual field is \emph{contiguous} and limited to dimensions similar to a human, which corresponds to limitations in the opening angle: horizontal $\leq$ \SI{180}{\degree} \improvement{TBD - feedback to the rules?} and vertical $\leq$ \SI{140}{\degree}.
...
The number of microphones is limited to 8. \todo{ok?}.
...
\subsubsection{Acoustic and Visual Communication}
\label{sec:acoustic}
...
\paragraph{Robot-To-Robot Communication:}
...
Each message's payload size must not exceed 512 Bytes. 
...
The number and the size of the messages sent by each team is monitored by the Game-Controller.

A violation of the permitted maximal size of a message or the maximal number of messages is subject to penalty.
\todo[inline]{What is the penalty?}
...
Robots are permitted to send status update packets to the GameController.

\todo[inline]{describe GC communication}


\paragraph{Debug Communication:}

Team specific debug information may be sent to an external computer owned by the team. 
A robot may send debug information at most once every second in a single UDP packet with the size of at most 64kB.


\section{Law 4 -- The Players' Equipment}
...
Each robot has a unique jersey number from the set $\{1, 2, 3, \ldots, 20\}$. \improvement[inline]{Will we have flexible numbers on the game controller? Otherwise, the numbers should be limited to the game controller numbers.}
...
Sponsorship logos will be allowed on jerseys, though detailed guidelines are still being finalized. \todo[inline]{to be discussed}
