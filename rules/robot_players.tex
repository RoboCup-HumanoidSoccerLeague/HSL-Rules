...
\subsection{Law 3 -- Teams and Players}
\label{sec:teams_and_players}

subsubsection{Divisions and Configurations}
\label{sec:divisions}
...
\subsubsection{Number of Substitutes}
...
\subsubsection{Substitution Procedure}
\label{sec:substitution_procedure}

In all matches, the names and jersey numbers of the substitutes must be given to the referee prior to the start of the match. Any substitute whose name is not given to the referee at this time may not take part in the match.
...
\paragraph{Safety}\info{From the humanoid league rules, to be discussed}
...
\subsubsection{Approved Standard Platforms}
...
The number of microphones is limited to \todo{To be defined. Suggested: 8}.
...
% AP
Wireless robot-to-robot communication between the robot players is allowed and must the access points provided by the event organizers. 
...
\subsection{Law 4 -- The Players' Equipment (missing!)}